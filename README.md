# 💰 Projeto ContaBancaria

Bem-vindo(a) ao meu projeto **ContaBancaria**! 🚀  
Este é um sistema simples desenvolvido em **Java** no **IntelliJ IDEA**, criado com o objetivo de praticar conceitos fundamentais da linguagem.

---

## 🧩 Funcionalidades

✨ Criação de conta bancária  
💵 Depósito de valores  
💸 Saque de valores  
📊 Exibição de saldo e informações da conta  

---

## 🧠 Tecnologias utilizadas

- ☕ **Java**
- 🧰 **IntelliJ IDEA**

---

## 🧪 Como executar o projeto
Main.java

=== Conta Bancária ===

Digite o nome do titular: Suelem martins

Digite o valor inicial: 2.500

Deposite um valor: 200.00

Saque um valor: 100.00

Saldo atual: 600.00


💡 Aprendizados

Durante o desenvolvimento, pratiquei:

Criação de classes e objetos 🧱

Uso de construtores e métodos

Entrada e saída de dados com Scanner

Manipulação de variáveis e lógica básica

---

1. Clone este repositório:
   ```bash
   https/github.com/SuelemMartins/conta_bancaria.git

## 🌟 Deixe sua estrelinha!

Se você gostou do projeto, não esqueça de **dar uma estrelinha ⭐** para apoiar e acompanhar meus próximos projetos! 💖  

---

<div align="center">

🌸 **Obrigada por visitar meu projeto!** 🌸  
💌 Seu apoio me motiva a continuar aprendendo e criando 💕  

<img src="sakura.gif" width="220" alt="Sakura piscando fofinha">

<br>

✨ **Deixe uma estrelinha!** ✨  
⭐ 👉 [Clique aqui e deixe seu apoio!](#)

<p align="center">
  Feito com carinho por <b>Suelem Martins</b> 💖  
</p>

</div>

🧑‍💻 Desenvolvido por

 Suelem Carla Martins
📍 Blumenau - SC
📧 suelemnascinto@gmail.com
   
